...
q-point mesh         = 20 × 20 × 20 for production screening; denser reruns for flagged cases
dense mesh check     = 24 × 24 × 24 after a strain-converged fallback
...
- 位移幅度；
- 超胞矩阵；
- 固定晶胞 BatchRelaxer/串行模式和 batch 原子上限；
- primitive matrix；
- q 网格；
- 松弛阈值；
- 温度范围；
...
- `--displacement 0.01`；
- `--mesh 20 20 20`；极端 γ、应变诱导虚频或网格敏感材料再复算更密网格；
- `--dense-mesh 24 24 24`；应变回退通过后必须完成更密网格复核；
...
- 软模和应变诱导虚频的处理。
- \(20^3\) 与 \(24^3\) 网格收敛以及应变收敛但网格不收敛的反例。
...
9. 默认生产普筛参数为：1M、0.01 Å、\(2^3\) 或 ≥12 Å、\(20^3\)；应变回退通过后必须完成 \(24^3\) 复核；
10. 缓存 fingerprint 必须包含输入哈希、执行模式、实际超胞和依赖版本；
11. 永不覆盖权威弹性张量；
12. 全量运行前必须完成代表材料收敛验证。

## 19. 全量生产前确认结果

1. 342 个目录均已建立 `elastic/calculation_metadata.json`；
2. 每个可运行目录均保存生成张量所对应的 `elastic/POSCAR`；
3. 当前解析器已对全部 342 个 `ELASTIC_TENSOR` 完成统一预检；
4. 327 个目录通过，12 个弹性张量非正定，4 个轴映射失败，其中 1 个重叠；
5. 应变、网格、残余力/应力、热容权重和极端 γ 阈值均有代表材料证据；
6. v2 使用独立版本化输出，不覆盖旧五列表或权威弹性张量；
7. 全量生产仍应使用 327 个通过预检的材料清单，并保留 15 个阻断项。
